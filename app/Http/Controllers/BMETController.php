...
use App\Models\BMET; // Import your BMET model
   ...
   public function index()
{
    ...
    $bmetCards = BMET::where('user_id', auth()->id())->orderBy('created_at', 'desc')->get();
    
    return view('user.bmet.index', compact('bmetCards'));
}


    public function create()
    {
        return view('user.bmet.create', ['title' => 'Create BMET Smart Card']);
    }
...
{
    $user = $this->user;
    $fee = \App\Models\ServiceCharge::getCharge('bmet');
   ...
   try {
    ...
    public function show($id)
    {
        ...
        return view('user.bmet.update-template', compact('bmet', 'photoUrl', 'qrUrl'));
    }

    public function edit($id)
    {
        $bmet = BMET::findOrFail($id);
        return view('user.bmet.edit', compact('bmet'));
    }
...
$certificateNo = mt_rand(1000000, 9999999); // 7-digit random number
    ...
    {
        $bmet = BMET::findOrFail($id);
        return view('user.bmet.verify', compact('bmet'));
    }
    ...
    {
        $bmet = BMET::where('token', $token)->firstOrFail();
    ...
    {
        $bmet = BMET::where('clearance_id', $clearanceId)->firstOrFail();
