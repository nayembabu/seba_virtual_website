        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h4 class="card-title">Passport Orders</h4>
                    <a href="{{ route("admin.passport-orders.export", ["format" => "csv"]) }}" class="btn btn-info btn-sm me-2">Export CSV</a>
                    <a href="{{ route("admin.passport-orders.export", ["format" => "pdf"]) }}" class="btn btn-info btn-sm">Export PDF</a>
                </div>
                <div class="card-body">
                    @if (session("success"))
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered" id="passportOrdersTable">
                            <thead>
                                <tr>
                                    <th>ID</th>
                                    <th>Name</th>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <p><strong>Passport No:</strong> <span id="detailPassportNo"></span></p>
...
<script>
    $(document).ready(function() {
        var passportOrdersTable = $("#passportOrdersTable").DataTable({
            processing: true,
            serverSide: true,
            responsive: true,
            ajax: {
                url: "{{ route("admin.passport-orders.get-orders") }}",
                type: "GET",
                data: function(d) {
                ...
                },
                {
                    ...
                    render: function(data, type, row) {
                        ...
                        if (row.status_code === 0) { // Pending
                            ...
                            buttons += `
                                <a href="/admin/passport-orders/${row.id}/download-pdf" class="btn btn-sm btn-primary" target="_blank">Download PDF</a>
                                <button type="button" class="btn btn-sm btn-secondary save-notes-btn" data-id="${row.id}" data-notes="${row.text || ''}" data-bs-toggle="modal" data-bs-target="#saveNotesModal">Notes</button>
                            `;
                        }
        ...
        passportOrdersTable.on("xhr.dt", function (e, settings, json, xhr) {
        ...
        $("#passportOrdersTable").on("click", ".view-details-btn", function() {
            var orderId = $(this).data("id");
            $.ajax({
                url: `/admin/passport-orders/${orderId}/details`,
                type: "GET",
                success: function(response) {
                    // Populate modal with response data
                    $("#detailOrderId").text(response.id);
                    $("#detailName").text(response.name);
                    $("#detailPassportNo").text(response.passport_no);
        ...
        $("#passportOrdersTable").on("click", ".approve-btn", function() {
            var orderId = $(this).data("id");
            if (confirm("Are you sure you want to approve this order?")) {
                $.ajax({
                    url: `/admin/passport-orders/${orderId}/approve`,
                    type: "POST",
                    data: { _token: "{{ csrf_token() }}" },
                    success: function(response) {
                        if (response.success) {
                            alert(response.message);
                            passportOrdersTable.ajax.reload();
                        } else {
                            alert(response.message);
                        }
                    },
        ...
        $("#passportOrdersTable").on("click", ".reject-btn", function() {
            var orderId = $(this).data("id");
            $("#rejectOrderForm").attr("action", `/admin/passport-orders/${orderId}/reject`);
            $("#reject_reason").val("");
            $("#reject_notes").val("");
        });
        ...
        $("#passportOrdersTable").on("click", ".upload-pdf-btn", function() {
            var orderId = $(this).data("id");
            $("#uploadPdfForm").attr("action", `/admin/passport-orders/${orderId}/upload-pdf`);
            $("#pdf_file").val("");
        });
        ...
        $("#passportOrdersTable").on("click", ".save-notes-btn", function() {
            var orderId = $(this).data("id");
            var notes = $(this).data("notes");
            $("#saveNotesForm").attr("action", `/admin/passport-orders/${orderId}/save-notes`);
