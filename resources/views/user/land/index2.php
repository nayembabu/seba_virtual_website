    ...
    <div class="card card-primary m-0 m-md-4 my-4 m-md-0 shadow">
        <div class="card-body bn-layout">
            <div class="text-right">
                <a href="{{route('user.land.create')}}" class="btn btn-success"> <i class="fas fa-plus"></i> কর দিন </a>
            </div>
                    ...
                    @forelse($objects as $key => $data)
                        <tr>
                            ...
                            <td data-label="@lang('অ্যাকশান')" class="text-lg-center text-right">
                                <!-- Print Button -->
                                <a href="{{ route('user.land.print', $data->uid) }}" class="btn btn-primary" target="_blank">
                                    <i class="fas fa-print"></i>
                                </a>

                                <!-- Edit Button -->
                                <a href="{{ route('user.land.update', $data->uid) }}" class="btn btn-info">
                                    <i class="fas fa-pencil-alt"></i>
                                </a>

                                <!-- Delete Button -->
                                <form style="display:inline-block" action="{{ route('user.land.delete', $data->uid) }}" method="post" onsubmit="return confirm('Do you really want to delete this?');">
                                    @csrf
