...
:root { --page-width: 210mm; --page-height: 297mm; --font: 'Nikosh', sans-serif; --base: 11px; --qr: 80px; --sig: 175px; --wm: 500px; }
* { box-sizing:border-box; margin:0; padding:0; }
body { font-family:var(--font) !important; font-size:var(--base); line-height:1.3; color:#000; background:#e5e7eb; }
.a4-page { width:var(--page-width); min-height:var(--page-height); padding:10mm 8mm; margin:10px auto; background:#fff; box-shadow:0 5px 15px rgba(0,0,0,.2); position:relative; }
.watermark { position:absolute; top:45%; left:50%; transform:translate(-50%,-50%); width:var(--wm); opacity:.08; z-index:1; }
...
@section('content')
<div class="btn-bar">
    <button onclick="window.print()" class="btn-dcr"><i class="fas fa-print"></i> প্রিন্ট</button>
    <button onclick="downloadPDF()" class="btn-dcr" style="background:#dc3545;"><i class="fas fa-file-pdf"></i> PDF ডাউনলোড</button>
    <a href="{{ route('user.dcr.logs') }}" class="btn-dcr" style="background:#6c757d;"><i class="fas fa-list"></i> লিস্ট</a>
</div>
        ...
        <table style="width:100%;margin-top:10px;border:none;">
            <tr>
                <td style="width:35%;text-align:center;vertical-align:middle;border:none;">
                    @if($dcr->unique_code)
                    <img src="data:image/png;base64,{{ $dcr->unique_code }}" style="width:var(--qr);padding:1px;" alt="QR">
                    <p style="margin-top:0;font-size:10px;">ভেরিফাই কোড</p>
                    @endif
                </td>
                <td style="width:30%;text-align:center;vertical-align:middle;border:none;">
                    @if($dcr->office_logo)
                    <img src="{{ asset($dcr->office_logo) }}" style="width:var(--qr);" alt="Logo">
                    @endif
                </td>
                <td style="width:35%;text-align:center;vertical-align:middle;border:none;">
                    <p style="margin-bottom:2px;">স্বাক্ষর</p>
                    @if($dcr->signature_img)
                    <div style="height:40px;"><img src="{{ asset($dcr->signature_img) }}" style="width:var(--sig);" alt=""></div>
                    @endif
...
<script>
function downloadPDF(){var e=document.getElementById("printableArea");Swal.fire({title:"PDF...",text:"Please wait...",allowOutsideClick:false,didOpen:function(){Swal.showLoading()}});html2pdf().set({margin:0,filename:"DCR_{{ $dcr->dcr_no }}.pdf",image:{type:"jpeg",quality:1},html2canvas:{scale:2,useCORS:true},jsPDF:{unit:"mm",format:"a4",orientation:"portrait"}}).from(e).save().then(function(){Swal.close()});}
</script>
@endpush
