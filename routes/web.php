...
Route::get('/user/visa-search', [\App\Http\Controllers\User\VisaSearchController::class, 'index'])->name('visa.search');
Route::post('/user/visa-search', [\App\Http\Controllers\User\VisaSearchController::class, 'searchVisa'])->name('visa.search.submit');

Route::get('/user/bmet-search', [App\Http\Controllers\User\BmetSearchController::class, 'index'])->name('bmet.search');
Route::post('/user/bmet-search', [App\Http\Controllers\User\BmetSearchController::class, 'searchBmet'])->name('bmet.search.submit');



// === Missing routes added by restore ===
Route::get('/user/manual-recharge', function() { 
    $user = Auth::user(); 
    try { $gateways = \App\Models\Gateway::where('status', 1)->get(); } catch (\Exception $e) { $gateways = collect(); }
    $min_d = 100;
    if ($gateways->count() > 0) { try { $min_d = $gateways->min('min_amount') ?? 100; } catch (\Exception $e) {} }
    $gw = $gateways->first();
    $recharges = new \Illuminate\Pagination\LengthAwarePaginator([], 0, 20);
    if ($user) { try { $recharges = \App\Models\Recharge::where('user_id', $user->id)->orderBy('id', 'desc')->paginate(20); } catch (\Exception $e) { $recharges = new \Illuminate\Pagination\LengthAwarePaginator([], 0, 20); } }
    return view('user.manual-recharge', compact('gateways', 'min_d', 'gw', 'recharges')); 
})->name('user.manual-recharge'); 
Route::post('/user/manual-recharge', function() { 
    $request = request();
    try {
        $data = [
            'user_id' => auth()->id(),
            'amount' => $request->input('amount'),
            'gateway_id' => $request->input('method'),
            'from' => $request->input('sender'),
            'txid' => $request->input('trxid'),
            'status' => 'pending',
        ];
        if ($request->hasFile('screenshot')) {
            $file = $request->file('screenshot');
            $filename = uniqid() . '.' . $file->getClientOriginalExtension();
            $file->move(public_path('assets/uploads/recharges'), $filename);
            $data['screenshot'] = $filename;
        }
        \App\Models\Recharge::create($data);
        return redirect()->route('user.manual-recharge')->with('success', 'রিচার্জ রিকোয়েস্ট সফলভাবে জমা দেওয়া হয়েছে। অ্যাডমিন অ্যাপ্রুভের অপেক্ষায় থাকুন।');
    } catch (\Exception $e) {
        return redirect()->route('user.manual-recharge')->with('error', 'সাবমিট করতে সমস্যা হয়েছে: ' . $e->getMessage());
    }
})->name('user.manual-recharge.submit');
Route::post('/user/manual-recharge/process', function() { 
    $request = request();
    $method = $request->input('method');
    $amount = $request->input('amount');
    if (!$method || !$amount) { return redirect()->route('user.manual-recharge')->with('error', 'পেমেন্ট মেথড এবং পরিমাণ নির্বাচন করুন।'); }
    $gateway = \App\Models\Gateway::where('name', $method)->where('status', 1)->first();
    if (!$gateway) { return redirect()->route('user.manual-recharge')->with('error', 'সিলেক্টেড গেটওয়ে সচল নেই।'); }
    $gatewayNumber = $gateway->account ?? 'Not Set';
    return view('user.manual-recharge-pay', compact('method', 'amount', 'gatewayNumber'));
})->name('user.manual-recharge.process');
Route::get('/user/mark-sheet', [\App\Http\Controllers\User\MarkSheetController::class, 'index'])->name('user.mark_sheet.index');
Route::get('/user/mark-sheet/create', [\App\Http\Controllers\User\MarkSheetController::class, 'create'])->name('user.mark_sheet.create');
Route::post('/user/mark-sheet', [\App\Http\Controllers\User\MarkSheetController::class, 'store'])->name('user.mark_sheet.store');
Route::get('/user/mark-sheet/{id}', [\App\Http\Controllers\User\MarkSheetController::class, 'show'])->name('user.mark_sheet.show');
Route::get('/user/mark-sheet/{id}/edit', [\App\Http\Controllers\User\MarkSheetController::class, 'edit'])->name('user.mark_sheet.edit');
Route::put('/user/mark-sheet/{id}', [\App\Http\Controllers\User\MarkSheetController::class, 'update'])->name('user.mark_sheet.update');
Route::delete('/user/mark-sheet/{id}', [\App\Http\Controllers\User\MarkSheetController::class, 'destroy'])->name('user.mark_sheet.destroy');
Route::get('/user/cv-maker', [\App\Http\Controllers\User\CvMakerController::class, 'index'])->name('user.cv-maker.index')->middleware('auth');
Route::post('/user/cv-maker/preview', [\App\Http\Controllers\User\CvMakerController::class, 'preview'])->name('user.cv-maker.preview');
Route::post('/user/cv-maker', [\App\Http\Controllers\User\CvMakerController::class, 'store'])->name('user.cv-maker.store');
Route::get('/user/cv-maker/{id}', [\App\Http\Controllers\User\CvMakerController::class, 'view'])->name('user.cv-maker.view');
Route::put('/user/cv-maker/{id}', [\App\Http\Controllers\User\CvMakerController::class, 'update'])->name('user.cv-maker.update');
Route::get('/user/dncc-trade', function() { return app(\App\Http\Controllers\User\TradeLicenseController::class)->index('dncc'); })->name('user.dncc-trade.index');
Route::get('/user/dncc-trade/generate', function() { return app(\App\Http\Controllers\User\TradeLicenseController::class)->generateLicenseNo('dncc'); })->name('user.dncc-trade.generate');
Route::post('/user/dncc-trade/submit', [\App\Http\Controllers\User\TradeLicenseController::class, 'submit'])->name('user.dncc-trade.submit');
Route::get('/user/dncc-trade/print/{id}', [\App\Http\Controllers\User\TradeLicenseController::class, 'print'])->name('user.dncc-trade.print');
Route::get('/user/dscc-trade', function() { return app(\App\Http\Controllers\User\TradeLicenseController::class)->index('dscc'); })->name('user.dscc-trade.index');
Route::get('/user/dscc-trade/generate', function() { return app(\App\Http\Controllers\User\TradeLicenseController::class)->generateLicenseNo('dscc'); })->name('user.dscc-trade.generate');
Route::post('/user/dscc-trade/submit', [\App\Http\Controllers\User\TradeLicenseController::class, 'submit'])->name('user.dscc-trade.submit');
Route::get('/user/dscc-trade/print/{id}', [\App\Http\Controllers\User\TradeLicenseController::class, 'print'])->name('user.dscc-trade.print');
Route::get('/user/certificate', [\App\Http\Controllers\User\CertificateController::class, 'index'])->name('user.certificate.index');
Route::post('/user/certificate/generate', [\App\Http\Controllers\User\CertificateController::class, 'generateNo'])->name('user.certificate.generate');
Route::post('/user/certificate/submit', [\App\Http\Controllers\User\CertificateController::class, 'submit'])->name('user.certificate.submit');
Route::get('/user/certificate/download/{id}', [\App\Http\Controllers\User\CertificateController::class, 'download'])->name('user.certificate.download');
Route::get('/user/electricity-bill', [\App\Http\Controllers\User\ElectricityBillController::class, 'index'])->name('user.electricity_bill.index');
Route::get('/user/electricity-bill/create', [\App\Http\Controllers\User\ElectricityBillController::class, 'create'])->name('user.electricity_bill.create');
Route::post('/user/electricity-bill', [\App\Http\Controllers\User\ElectricityBillController::class, 'store'])->name('user.electricity_bill.store');
Route::get('/user/electricity-bill/{id}/edit', [\App\Http\Controllers\User\ElectricityBillController::class, 'edit'])->name('user.electricity_bill.edit');
Route::put('/user/electricity-bill/{id}', [\App\Http\Controllers\User\ElectricityBillController::class, 'update'])->name('user.electricity_bill.update');
Route::delete('/user/electricity-bill/{id}', [\App\Http\Controllers\User\ElectricityBillController::class, 'destroy'])->name('user.electricity_bill.destroy');
Route::get('/user/electricity-bill/{id}', [\App\Http\Controllers\User\ElectricityBillController::class, 'show'])->name('user.electricity_bill.show');
Route::get('/user/dcr/create', [\App\Http\Controllers\User\DcrController::class, 'create'])->name('user.dcr.create');
Route::post('/user/dcr', [\App\Http\Controllers\User\DcrController::class, 'store'])->name('user.dcr.store');
Route::get('/user/dcr/logs', [\App\Http\Controllers\User\DcrController::class, 'logs'])->name('user.dcr.logs');
Route::get('/user/dcr/{id}', [\App\Http\Controllers\User\DcrController::class, 'view'])->name('user.dcr.view');
Route::get('/user/dcr/{id}/download', [\App\Http\Controllers\User\DcrController::class, 'downloadPdf'])->name('user.dcr.download');

Route::get('/user/golden-card', [\App\Http\Controllers\User\GoldenCardController::class, 'index'])->name('user.golden-card.index');
Route::post('/user/golden-card/generate', [\App\Http\Controllers\User\GoldenCardController::class, 'generateCardNo'])->name('user.golden-card.generate');
Route::post('/user/golden-card/submit', [\App\Http\Controllers\User\GoldenCardController::class, 'submit'])->name('user.golden-card.submit');
Route::get('/user/golden-card/print/{id}', [\App\Http\Controllers\User\GoldenCardController::class, 'print'])->name('user.golden-card.print');
Route::get('/user/khatian/create', [\App\Http\Controllers\User\KhatianController::class, 'create'])->name('user.khatian.create');
Route::post('/user/khatian', [\App\Http\Controllers\User\KhatianController::class, 'store'])->name('user.khatian.store');
Route::get('/user/khatian/logs', [\App\Http\Controllers\User\KhatianController::class, 'logs'])->name('user.khatian.logs');
Route::post('/user/khatian', function() { return back()->with('success', 'Processing...'); })->name('user.khatian.store');
// === End missing routes ===
...
Route::group(['middleware' => ['auth', 'userCheck'], 'prefix' => 'user', 'as' => 'user.'], function () {
    ...
    Route::get('/recharge/zinipay', [\App\Http\Controllers\User\ZinipayController::class, 'index'])->name('zinipay');
    Route::post('/recharge/zinipay', [\App\Http\Controllers\User\ZinipayController::class, 'pay'])->name('zinipay');
    Route::get('/recharge/zinipay-callback', [\App\Http\Controllers\User\ZinipayController::class, 'callback'])->name('zinipay-callback');
    Route::post('/recharge/zinipay-webhook', [\App\Http\Controllers\User\ZinipayController::class, 'webhook'])->name('zinipay-webhook')->withoutMiddleware(\App\Http\Middleware\VerifyCsrfToken::class)->withoutMiddleware(\App\Http\Middleware\Authenticate::class)->withoutMiddleware(\App\Http\Middleware\CheckUserStatus::class);
    ...
    // 13. eVisa
    Route::resource('evisas', EVisaController::class);
...
Route::group(['middleware' => ['auth', 'userCheck'], 'prefix' => 'user', 'as' => 'user.'], function () {
    ...
    Route::get('/server-copy', 'User\HomeController@server_copy')->name('server-copy');
    Route::post('/server-copy', 'User\HomeController@server_copy_post')->name('server-copy');
    Route::get('/server-copy-view', 'User\HomeController@server_copy_view')->name('server-copy-view');
